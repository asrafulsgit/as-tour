...
const initPaymentService = async (bookingId: string) => {
  ...
  if (booking?.status !== BOOKING_STATUS.PENDING) {
    throw new AppError(httpStatus.BAD_REQUEST, `Booking ${booking?.status}`);
  }
...
const getInvoiceService = async (paymentId: string, userId: string) => {
  ...
  if (
    String(userId) !== String((payment?.booking as unknown as IBooking).user)
  ) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "You are not allow to get invoice",
    );
  }
